...
class TablioSupabaseUser extends BaseAuthUser {
  TablioSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> tablioSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = TablioSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
